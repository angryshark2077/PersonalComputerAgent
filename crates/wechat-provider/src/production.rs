...
fn diagnostic_recent_rows(
    ...
    for row in rows.flatten() {
        ...
        let emoticon_md5 = content
            .as_deref()
            .and_then(|value| xml_attribute(value, "md5"))
            .and_then(|value| valid_md5(&value));
        let emoticon_key = content
            .as_deref()
            .and_then(|value| xml_attribute(value, "aeskey"));
        let emoticon_cached = emoticon_md5
            .as_ref()
            .is_some_and(|md5| emoticon_files.contains_key(md5));
        let emoticon_decrypted = emoticon_md5
            .as_ref()
            .and_then(|md5| emoticon_files.get(md5))
            .zip(emoticon_key.as_ref())
            .is_some_and(|(path, key)| {
                fs::read(path)
                    .ok()
                    .and_then(|bytes| decrypt_emoticon(&bytes, key))
                    .is_some()
            });
        eprintln!(
            "ROW conversation={target} sequence={} time={} local_type={} status={} server_id={} decoded={} app_type={} file_name={} location={} video_key={} video_md5s={} video_length={} video_cdn={} video_aes_key={} hardlink_keys={} video_resolved={} emoticon_cached={} emoticon_key_length={} emoticon_decrypted={}",
            row.0,
            row.1,
            row.2,
            row.3,
            ...
            video_resolved,
            emoticon_cached,
            emoticon_key.as_ref().map_or(0, String::len),
            emoticon_decrypted,
        );
    }
    Ok(())
...
fn read_database_text(
    ...
    for session in sessions {
        ...
        } else if is_direct_conversation(&session.username, &metadata.display_name) {
...
#[allow(
    clippy::too_many_lines,
    reason = "emoticon rows share the reviewed image identity and scope mapping"
)]
fn read_database_emoticons(
    connection: &Connection,
    context: &TextReadContext<'_>,
    sessions: &[Session],
    emoticon_files: &BTreeMap<String, PathBuf>,
    ...
    let mut cursor_updates = Vec::new();
    let mut cursor_removals = Vec::new();
    let diagnostic_target = std::env::var("PCA_WECHAT_DIAGNOSTIC_CONVERSATION")
        .ok()
        .filter(|_| std::env::var_os("PCA_WECHAT_ROW_DIAGNOSTIC").is_some());
    for session in sessions {
        if diagnostic_target
            .as_ref()
            .is_some_and(|target| target != &session.username)
        {
            continue;
        }
        if records.len() >= limit {
...
fn read_database_files(
    ...
    for session in sessions {
        ...
        let cursor_key = format!(
            "{}:{}:file-v2",
            read_context.database_name, session.username
        );
        ...
        cursor_removals.extend(
            retry_rows
                .iter()
                .filter(|(_, _, create_time)| *create_time < read_context.cutoff)
                .map(|(key, _, _)| key.clone()),
        );
        let retry_ids = retry_rows
            .iter()
            .filter(|(_, _, create_time)| *create_time >= read_context.cutoff)
            .take(MAX_PENDING_FILE_RETRIES_PER_CONVERSATION)
            .map(|(_, local_id, _)| *local_id)
            .collect::<BTreeSet<_>>();
        let per_conversation = MAX_PER_CONVERSATION.min(limit - records.len());
        let retry_clause = if retry_ids.is_empty() {
        ...
        let sql = format!(
            "SELECT local_id, server_id, create_time, real_sender_id, status, message_content, compress_content \
             FROM \"{table_name}\" \
             WHERE (local_type & 4294967295) = 49 \
               AND (local_id > ?1{retry_clause}) AND create_time >= ?2 \
             ORDER BY local_id ASC"
        );
        let mut statement = connection
        ...
        for row in rows {
            ...
            let completed_file =
                resolve_message_file(message_files, &file_name, expected_size, row.create_time)
                    .and_then(|source| {
                        file_attachment(
                            source,
            ...
            let Some((attachment, source_path)) = completed_file else {
                if !is_retry {
                    ...
                    records.push(SourceMessageRecord {
                        ...
                        sender_avatar_url,
                        source_key: format!(
                            "wechat:{}:{table_name}:{}:file-pending",
...
fn resolve_message_file<'a>(
    ...
    if let Some(candidate) = candidates.iter().find(|path| {
        ...
        return Some(candidate.as_path());
    }
    let create_time = u64::try_from(create_time).ok()?;
    let mut nearby = candidates.iter().filter(|path| {
        path.symlink_metadata()
            .ok()
            .and_then(|metadata| metadata.modified().ok())
            .and_then(|modified| modified.duration_since(UNIX_EPOCH).ok())
            .is_some_and(|modified| {
                modified.as_secs().abs_diff(create_time) <= FILE_TIMESTAMP_FALLBACK_SECONDS
            })
    });
    let candidate = nearby.next()?;
    nearby.next().is_none().then_some(candidate.as_path())
}
...
fn decrypt_emoticon_with_key(encrypted: &[u8], key: &[u8; 16]) -> Option<Vec<u8>> {
    ...
    let padding = usize::from(*decrypted.last()?);
    if padding == 0
        || padding > 16
    ...
    normalize_decrypted_image(decrypted)
}

fn emoticon_attachment(
    source: &Path,
    aes_key: &str,
    database_name: &str,
    table_name: &str,
    ...
    let decrypted = decrypt_emoticon(&encrypted, aes_key)?;
    let mime_type = image_mime_type(&decrypted)?.to_owned();
    let sha256 = format!("{:x}", Sha256::digest(&decrypted));
    let source_path = stage_decrypted_image(&decrypted, &sha256, &mime_type)?;
    let table_hash = table_name.strip_prefix("Msg_")?;
    let attachment_id = format!("wechat-emoticon:{database_name}:{table_hash}:{local_id}");
    let attachment = CommunicationAttachment::try_new(
        attachment_id,
        MessageKind::Image,
        sha256,
...
fn is_direct_conversation(username: &str, display_name: &str) -> bool {
    const SYSTEM: [&str; 10] = [
        "fmessage",
        "floatbottle",
        "medianote",
...
mod tests {
    ...
    #[test]
    fn production_retries_a_file_that_materializes_after_its_message_row() {
        let connection = Connection::open_in_memory().expect("open delayed file fixture");
        let conversation_id = "wxid_friend";
        let table_name = message_table_name(conversation_id);
        connection
            .execute_batch(&format!(
                "CREATE TABLE Name2Id (user_name TEXT);\
                 ...
                 INSERT INTO \"{table_name}\" VALUES (7, 107, 1000, 1, 0, 49,\
                    '<appmsg><type>6</type><title>report.xlsx</title><totallen>4</totallen></appmsg>', X'');"
            ))
            .expect("create delayed file schema");
        let account = tempfile::tempdir().expect("create delayed file source");
        let metadata = std::collections::BTreeMap::from([(
            conversation_id.to_owned(),
            ConversationMetadata {
                display_name: "Friend".to_owned(),
        ...
        assert!(matches!(
            &first.records[0].payload,
            SourcePayload::Text { body } if body == "[文件] 等待微信保存原始文件"
        ));
        let retry_key = format!("message_0.db:{conversation_id}:file-v2:pending:7");
        assert!(first.cursor_updates.contains(&(retry_key.clone(), 1000)));

        let directory = account.path().join("msg/file/1970-01");
        std::fs::create_dir_all(&directory).expect("create delayed file directory");
        std::fs::write(directory.join("report.xlsx"), b"xlsx").expect("materialize delayed file");
        let retry_cursors = first.cursor_updates.into_iter().collect();
        let retry_context = TextReadContext {
            cursors: &retry_cursors,
            ..context
        ...
        assert_eq!(second.records.len(), 1);
        assert!(matches!(
            second.records[0].kind,
            crate::source::SourceMessageKind::File
        ));
        assert_eq!(second.cursor_removals, vec![retry_key]);
    }

    #[test]
    fn production_accepts_a_unique_recent_file_when_wechat_reports_the_wrong_size() {
        let fixture = tempfile::tempdir().expect("create recent file fixture");
        let file = fixture.path().join("report.xlsx");
        std::fs::write(&file, b"actual file bytes").expect("write recent file");
        let create_time = i64::try_from(
        ...
        let files =
            std::collections::BTreeMap::from([("report.xlsx".to_owned(), vec![file.clone()])]);

        let resolved = resolve_message_file(&files, "report.xlsx", Some(4), create_time);
    ...
    #[test]
    fn production_decrypts_aes_ecb_emoticon_cache() {
        let key = [
        ...
        assert_eq!(
            decrypt_emoticon(&padded, "00112233445566778899aabbccddeeff"),
            Some(plain.to_vec())
        );
        assert!(decrypt_emoticon(&padded, "not-a-key").is_none());
    }

    #[test]
    fn production_reads_cached_emoticon_as_an_image() {
        let fixture = tempfile::tempdir().expect("create emoticon fixture");
        let key = [
            0x00, 0x11, 0x22, 0x33, 0x44, 0x55, 0x66, 0x77, 0x88, 0x99, 0xaa, 0xbb, 0xcc, 0xdd,
            0xee, 0xff,
        ];
        ...
        fs::write(&cache, encrypted).expect("write encrypted emoticon");
        let md5 = "a55283fb8ef3bb36bbb959a5c073d875";
        let emoticons = std::collections::BTreeMap::from([(md5.to_owned(), cache)]);

        let connection = Connection::open_in_memory().expect("open emoticon database");
        let conversation_id = "wxid_friend";
        let table_name = message_table_name(conversation_id);
        connection
            .execute_batch(&format!(
            ...
            .expect("create emoticon message fixture");
        let metadata = std::collections::BTreeMap::from([(
            conversation_id.to_owned(),
            ConversationMetadata {
                display_name: "Friend".to_owned(),
        ...
        assert_eq!(batch.records.len(), 1);
        assert_eq!(batch.records[0].source_sequence, 63);
        assert!(matches!(
            batch.records[0].payload,
            SourcePayload::Media {
                attachment: Some(_),
                completed_source: Some(_)
            }
        ));
        assert_eq!(
            batch.cursor_updates,

...
export function renderDeviceConfiguration(device: DashboardDevice): string {
  return [
    ...
    `<section aria-label="WeChat messages"><h2>WeChat messages</h2><p>Incoming and outgoing text, audio, images, video and files; direct chats and groups up to 15 members; 180-day retention</p><p>${enabledLabel(device.collectors["communication.wechat"].enabled)}</p></section>`,
    ...
    `<section aria-label="Photos"><h2>Photos</h2><p>Original photos and videos with capture time and album names; initial 7-day history and future items; permanent Cloud retention</p><p>${enabledLabel(device.collectors["photos.library"].enabled)}</p></section>`,
  ].join("");
}
